...
- **Reports location every 5 minutes** via a persistent foreground service (not WorkManager — its periodic-task floor is 15 minutes), or immediately on demand via "Check in now" on the Home screen.
...
Two independent triggers feed the same `SecurityCaptureService`, which takes one front-camera photo, reads the current location, encrypts both exactly like an ordinary location check-in, and uploads them (`POST /security_events.php`) using the device's own scoped token — no interactive login required, so it works even if whoever failed the attempt never gets past that screen. The account owner reviews these later from the device's own screen — the shield icon next to "Play sound" in the app's device-history view, or the "Security log" button under a device in the web viewer — the same way a bank app logs failed sign-in attempts.
